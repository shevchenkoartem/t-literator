...
// DEPRECATED: Use the appropriate TransliteratorConfig method instead?
    ...
    #replaceAllByDict(src, dict, useLocationInWordAlgo) {
        return Object.entries(dict).reduce((res, [key, vals]) => {
            if (useLocationInWordAlgo && Array.isArray(vals)) {
                res = res.replaceAll(this.#WORD_START + key, this.#WORD_START + CfgHelpers.getPositionalValue_Pre(vals));
                res = res.replaceAll(key + this.#WORD_END, CfgHelpers.getPositionalValue_Post(vals) + this.#WORD_END);
            }
            return res.replaceAll(key, CfgHelpers.getPositionalValue_Mid(vals));
        }, src);
    }
}
